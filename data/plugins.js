  ...
  remove: 
    function (id, values, mimetype) {
      ...
      for (var i=0; i<DwbPlugin._plugins.length; i++) {
        if (DwbPlugin._plugins[i] == id) {
          return "__dwb__blocked__";
        }
      }
      DwbPlugin._plugins.push(id);
      var style = document.defaultView.getComputedStyle(e, null);
      var obj = document.createElement('div');
      obj.style.width = style.width;
      obj.style.height = style.height;
      ...
      obj.onclick  = function() { DwbPlugin.onClick(obj, id, values); };
      e.parentNode.appendChild(obj);
      e.parentNode.removeChild(e);
      return 0;
    },
  onClick: 
    function(obj, id, values) {
      if (!document.getElementById('dwb_plugin_' + id)) {
        obj.innerHTML = '<div id=dwb_plugin_' + id + '><embed ' + values + ' /></div>'; 
