const DWB_PLUGIN_BACKGROUND = "transparent url(data:image/png;base64,) no-repeat scroll center";

const DwbPlugin = {
  _plugins : [], 
  getElementById: 
    function (id) {
      for (var i=0; i<DwbPlugin._plugins.length; i++) {
        if (DwbPlugin._plugins[i].id == id) {
          return DwbPlugin._plugins[i];
        }
      }
      return null;
    },
  remove: 
    function (id, x, y, w, h) {
      var e = DwbPlugin.getElementById(id);

      var obj = document.createElement('div');
      var link = document.createElement('a');

      var element;
      link.href = "plugin:" + id;
      link.style.display = 'block';
      obj.style.border = "1px solid";
      if (e.element) {
        var style = e.element.getBoundingClientRect();
        e.w = style.width;
        e.h = style.height;
        e.e = e.element.cloneNode(true);
      }
      else {
        obj.style.position = 'absolute';
        e.x = parseInt(x) + window.pageXOffset;
        e.y = parseInt(y) + window.pageYOffset;
        obj.style.top = e.y + 'px';
        obj.style.left = e.x + 'px';
        e.w = w;
        e.h = h;
      }
      link.style.width = e.w + 'px';
      link.style.height = e.h + 'px';
      obj.zIndex = 1369;
        
      obj.style.background= DWB_PLUGIN_BACKGROUND;
      obj.appendChild(link);
      e.obj = obj;
      if (e.e) {
        e.element.parentNode.appendChild(e.obj);
        e.element.parentNode.removeChild(e.element);
      }
      else {
        document.body.appendChild(obj);
      }
      return 0;
    },
  init: 
    function(id, values) {
      if (DwbPlugin.getElementById(id)) 
          return "__dwb__blocked__";
      var element = document.getElementById(id);
      var e = { id:id, tagName : element ? element.tagName : 'embed', values: values, element:element };
      DwbPlugin._plugins.push(e);
      return 0;
    },
  click: 
    function(id) {
      var e = DwbPlugin.getElementById(id);
      if (e) {
        if (e.e) {
          e.obj.parentNode.appendChild(e.e);
        }
        else {
          var p = document.createElement('div');
          p.innerHTML = '<' + e.tagName + ' ' + e.values + ' />';
          p.style.width = e.w + 'px';
          p.style.height = e.h + 'px';
          p.style.position = 'absolute';
          p.style.top = e.y + 'px';
          p.style.left = e.x + 'px';
          document.body.appendChild(p);
        }
        e.obj.parentNode.removeChild(e.obj);
      }
    },
}
